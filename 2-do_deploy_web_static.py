...
def do_deploy(archive_path):
    ...
    try:
        ...
        run ("tar -xzf /tmp/{} -C /data/web_static/releases/{}/".format(file_withex, file_noex))
        run("rm /tmp/{}".format(file_withex))
        run("mv /data/web_static/releases/{0}/web_static/* /data/web_static/releases/{0}/".format(file_noex))
        run("rm -rf /data/web_static/releases/{}/web_static".format(file_noex))
        run("rm -rf /data/web_static/current")
        run("ln -s /data/web_static/releases/{}/ /data/web_static/current".format(file_noex))
